...
RestoredCollection:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, DonkeyJawbone.PostNewRoom)
...
RestoredCollection:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, DonkeyJawbone.PlayerHurt, EntityType.ENTITY_PLAYER)
...
RestoredCollection:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, DonkeyJawbone.JawboneUpdate, RestoredCollection.Enums.Entities.DONKEY_JAWBONE.Variant)

function DonkeyJawbone:SpawnJawbone(player)
	local jawbone = Isaac.Spawn(1000, RestoredCollection.Enums.Entities.DONKEY_JAWBONE.Variant, 0, player.Position, Vector.Zero, player):ToEffect()
	local data = Helpers.GetData(player)
